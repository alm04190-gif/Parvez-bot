...
TOKEN = "8500240073:AAHtfZalwKloNC2PPqkeQFIPaCKVw1H30gw"

OWNER_ID = 8293984966        # <-- রমার Telegram numeric ID
ADMIN_ID = 8293984966        # <-- চাইলে আলাদা admin ID দাও

YOUR_USERNAME = "@Parvezhacker"   # <-- রমার Telegram username
UPDATE_CHANNEL = "https://t.me/all_in_one_63"  # <-- Update channel link
...
pending_approvals = {}  # {message_id: {"user_id": int, "file_name": str, "file_path": Path, "file_type": str}}
...
def is_free_user(user_id):
    return user_id != OWNER_ID and user_id not in admin_ids and not is_premium_user(user_id)


# =============== HELPER: START A SCRIPT PROCESS ===============

def start_script_process(user_id: int, file_name: str, user_folder: Path):
    """Start a script subprocess and register it in bot_scripts. Returns process or None."""
    file_path = user_folder / file_name
    if not file_path.exists():
        logger.error(f"Cannot run: file not found: {file_path}")
        return None
    ...
    if script_key in bot_scripts:
        logger.info(f"Script {script_key} already running, skipping.")
        return None
    ...
    try:
        ...
        free = is_free_user(user_id)

        if free:
            def set_low_priority():
                try:
                    current_nice = os.nice(0)
                    increase = max(0, FREE_USER_NICE - current_nice)
                    if increase > 0:
                        os.nice(increase)
                except Exception:
                    pass
        ...
        if file_ext == '.py':
            ...
            log_file.close()
            logger.error(f"Cannot run file type: {file_ext}")
            return None
        ...
        try:
            ...
            logger.error(f"DB stat update error: {e}")

        logger.info(f"Script started: {script_key}, PID={process.pid}")
        return process

    except Exception as e:
        logger.error(f"Error starting script {file_name} for user {user_id}: {e}")
        return None
...
def get_main_keyboard(user_id):
    if user_id in admin_ids:
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="❤️ Favorites", callback_data="my_favorites"),
...
💎 <b>Account:</b> {'Premium ✅' if is_premium_user(user_id) else 'Free 🆓'}
...
❤️ <b>Add Favorites</b> - Quick access to files
🔍 <b>Search Files</b> - Find files easily
▶️ <b>Run Scripts</b> - Execute Python/JS code (Admin Approval Required)
...
All uploaded files require <b>Admin Approval</b> before running!
Your file will be sent to admin for review.
After approval, your bot will <b>start automatically</b>! 🚀

<b>📌 How to Upload:</b>

1️⃣ Send your file to the bot
2️⃣ Wait for admin approval
3️⃣ Bot starts automatically after approval!
...
"""
        ...
        for i, (file_name, file_type) in enumerate(files, 1):
            ...
            run_btn_text = "🛑 Stop" if is_running else "▶️ Run"
            run_btn_callback = f"stop_script:{script_key}" if is_running else f"run_script:{file_name}"
            
            buttons.append([
                InlineKeyboardButton(text=f"{run_btn_text} {file_name[:15]}", callback_data=run_btn_callback),
                InlineKeyboardButton(text=f"{star}", callback_data=f"toggle_fav:{file_name}")
            ])
            # --- CHANGED: Added 📋 Log button ---
            buttons.append([
                InlineKeyboardButton(text=f"ℹ️ Info", callback_data=f"file_info:{file_name}"),
                InlineKeyboardButton(text=f"📋 Log", callback_data=f"view_log:{file_name}"),
...
async def callback_view_log(callback: types.CallbackQuery):
    ...
    if not log_file_path.exists():
        await callback.answer("❌ No log file found! Run the script first.", show_alert=True)
        return
    ...
    try:
        ...
        # Show last 3500 chars to fit Telegram message limit
        if len(log_content) > 3500:
            log_content = "...(truncated - showing last lines)...\n" + log_content[-3500:]

        if not log_content.strip():
            log_content = "(No output yet — script may have just started)"
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔄 Refresh Log", callback_data=f"view_log:{file_name}")],
            [InlineKeyboardButton(text="📁 My Files", callback_data="check_files"),
             InlineKeyboardButton(text="🏠 Home", callback_data="back_to_main")]
        ])

        await callback.message.edit_text(text, reply_markup=keyboard, parse_mode="HTML")
        await callback.answer("✅ Log refreshed!")
...
"""
        ...
        text = f"""
╔═══════════════════════════╗
    ❤️ <b>FAVORITES ({len(favorites)})</b> ❤️
╚═══════════════════════════╝

"""
        buttons = []
        for i, file_name in enumerate(favorites, 1):
            text += f"{i}. ❤️ <code>{file_name}</code>\n"
            buttons.append([
...
2️⃣ <b>Wait for Approval:</b>
   • Admin will review your file
   • Once approved, bot starts automatically!
   • Check 'My Files' to see status

3️⃣ <b>Run Scripts:</b>
   • Go to 'My Files'
   • Click 'Run' on approved file
   • Monitor script execution

4️⃣ <b>View Logs:</b>
   • Go to 'My Files'
   • Click '📋 Log' on any file
   • See live output from your script

5️⃣ <b>Manage Files:</b>
   • View all files in 'My Files'
   • Add to favorites with ❤️
   • Delete unwanted files (stops running scripts)
...
3. 📁 View & Manage Files
4. ❤️ Add to Favorites
5. 🔍 Search Files by Name
6. ▶️ Run Python Scripts (After Approval)
7. ▶️ Run JavaScript Scripts (After Approval)
8. 🛑 Stop Running Scripts
9. 📊 View Your Statistics
10. ⚡ Bot Speed Test
11. 📥 Download Your Files
12. 💾 View File Information
13. 📋 View Live Logs
14. ℹ️ Help & Support
...
❤️ <b>Favorite:</b> {'Yes ✅' if is_favorite else 'No'}
...
"""
    ...
    back_keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📋 View Log", callback_data=f"view_log:{file_name}")],
        [InlineKeyboardButton(text="📁 My Files", callback_data="check_files"),
         InlineKeyboardButton(text="🏠 Home", callback_data="back_to_main")]
    ])
...
async def handle_document(message: types.Message):
    ...
    try:
        ...
        # Send file to admin for approval
        admin_keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [
                InlineKeyboardButton(text="✅ Confirm", callback_data=f"approve_file:{user_id}:{file_name}"),
                InlineKeyboardButton(text="❌ Reject", callback_data=f"reject_file:{user_id}:{file_name}")
            ]
        ])
        ...
        await bot.send_document(
            ...
            caption=(
                f"📤 <b>New File Upload Request</b>\n\n"
                f"👤 <b>User:</b> {message.from_user.full_name}\n"
                f"🆔 <b>User ID:</b> <code>{user_id}</code>\n"
                f"📄 <b>File:</b> <code>{file_name}</code>\n"
                f"💾 <b>Size:</b> {file_size_kb:.2f} KB\n\n"
...
Your file has been sent to admin for review. 
Once approved, your bot will <b>start automatically!</b> 🚀
...
@dp.callback_query(F.data.startswith("approve_file:"))
async def callback_approve_file(callback: types.CallbackQuery):
    ...
    try:
        ...
        auto_run_msg = ""
        run_file_name = None  # The actual script file to run

        # ---- ZIP: extract first, then find main script ----
        if file_ext == '.zip':
            if not file_path.exists():
                await callback.answer("❌ ZIP file not found on server!", show_alert=True)
                return
            ...
            try:
                extracted_scripts = []  # .py/.js files found inside
                conn = sqlite3.connect(DATABASE_PATH)
                c = conn.cursor()
                now = datetime.now().isoformat()
                ...
                with zipfile.ZipFile(file_path, 'r') as zip_ref:
                    ...
                    for entry in all_entries:
                        ...
                        just_name = entry_path.name          # flatten: only filename
                        entry_ext = entry_path.suffix.lower()

                        # Extract every file flat into user_folder (DB, configs, etc.)
                        dest_path = user_folder / just_name
                        with zip_ref.open(entry) as zf:
                            with open(dest_path, 'wb') as out_f:
                                out_f.write(zf.read())

                        # Register .py and .js files in tracking
                        if entry_ext in ['.py', '.js']:
                            if user_id not in user_files:
                                user_files[user_id] = []
                            existing_names = [f[0] for f in user_files[user_id]]
                            if just_name not in existing_names:
                                user_files[user_id].append((just_name, entry_ext[1:]))
                            c.execute(
                                'INSERT OR REPLACE INTO user_files (user_id, file_name, file_type, upload_date) VALUES (?, ?, ?, ?)',
                                (user_id, just_name, entry_ext[1:], now)
                            )
                            extracted_scripts.append(just_name)

                # Remove the zip from tracking
                ...
                # Determine which script to auto-run:
                # Priority 1 → main.py, Priority 2 → first .py, Priority 3 → first .js
                if 'main.py' in extracted_scripts:
                    run_file_name = 'main.py'
                else:
                    py_scripts = [f for f in extracted_scripts if f.endswith('.py')]
                    js_scripts = [f for f in extracted_scripts if f.endswith('.js')]
                    if py_scripts:
                        run_file_name = py_scripts[0]
                    elif js_scripts:
                        run_file_name = js_scripts[0]

                if run_file_name:
                    process = start_script_process(user_id, run_file_name, user_folder)
                    if process:
                        auto_run_msg = (
                            f"\n\n📦 ZIP extracted successfully!"
                            f"\n🚀 <b>Bot started automatically!</b>"
                            f"\n📄 Running: <code>{run_file_name}</code>"
                            f"\n🔢 PID: <code>{process.pid}</code>"
                        )
                        logger.info(f"ZIP approved & auto-started {run_file_name} for user {user_id}, PID={process.pid}")
                    else:
                        auto_run_msg = (
                            f"\n\n📦 ZIP extracted! Scripts found: {', '.join(extracted_scripts)}"
                            f"\n⚠️ Auto-start failed. Please start manually from My Files."
                        )
                else:
                    auto_run_msg = (
                        f"\n\n📦 ZIP extracted! No runnable .py/.js found."
                        f"\nExtracted files are available in your folder."
                    )

            except Exception as zip_err:
                logger.error(f"ZIP extraction error during approval: {zip_err}")
                auto_run_msg = f"\n\n❌ ZIP extraction failed: {str(zip_err)}"

        # ---- .py or .js: run directly ----
        elif file_ext in ['.py', '.js'] and file_path.exists():
            process = start_script_process(user_id, file_name, user_folder)
            if process:
                auto_run_msg = (
                    f"\n\n🚀 <b>Bot started automatically!</b>"
                    f"\n🔢 PID: <code>{process.pid}</code>"
                )
                logger.info(f"Auto-started {file_name} for user {user_id} after approval, PID={process.pid}")
            else:
                auto_run_msg = "\n\n⚠️ Auto-start failed. Please start manually from My Files."
...
🎉 Your file has been approved by admin.{auto_run_msg}

Click below to view your files 👇
""",
        ...
        await callback.message.edit_caption(
            caption=callback.message.caption + "\n\n✅ <b>APPROVED</b> by admin — Bot auto-started!",
            parse_mode="HTML"
        )

        await callback.answer("✅ File approved & bot started!", show_alert=True)

    except Exception as e:
        logger.error(f"Error approving file: {e}")
        await callback.answer(f"❌ Error: {str(e)}", show_alert=True)

@dp.callback_query(F.data.startswith("reject_file:"))
async def callback_reject_file(callback: types.CallbackQuery):
    if callback.from_user.id not in admin_ids:
        await callback.answer("❌ Admin only!", show_alert=True)
        return
...
😔 Sorry! Your file has been rejected by admin.
Please try uploading a different file.

If you think this is a mistake, contact support.
""",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📲 Contact Support", url=f"https://t.me/{YOUR_USERNAME.replace('@', '')}")],
                [InlineKeyboardButton(text="🏠 Main Menu", callback_data="back_to_main")]
            ]),
            parse_mode="HTML"
        )
        ...
        await callback.answer("❌ File rejected and deleted!", show_alert=True)
...
async def callback_run_script(callback: types.CallbackQuery):
    ...
    if process is None:
        await callback.answer("❌ Failed to start script!", show_alert=True)
        return

    if free:
        speed_note = "\n\n⚠️ <b>Slow Mode:</b> Your bot is running with reduced CPU priority (Free user)"
    else:
        speed_note = "\n\n⚡ <b>Fast Mode:</b> Your bot is running at full speed!"
...
"""
        ...
        # --- CHANGED: Flatten extraction — all files go to user_folder root ---
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            all_files_in_zip = zip_ref.namelist()
            for entry in all_files_in_zip:
                # Skip directories
                if entry.endswith('/'):
                    continue

                entry_path = Path(entry)
                just_name = entry_path.name  # Only the filename, no subdirectory path
                file_ext = entry_path.suffix.lower()

                # Write ALL files (including non-py/js like DBs, configs) to user_folder root
                dest_path = user_folder / just_name
                with zip_ref.open(entry) as zf:
                    with open(dest_path, 'wb') as out_f:
                        out_f.write(zf.read())

                # Register only .py and .js files in the bot's tracking
        ...
        # Remove the zip entry from tracking
        if user_id in user_files:
            user_files[user_id] = [f for f in user_files[user_id] if f[0] != file_name]
...
📊 <b>Total Extracted:</b> {len([e for e in all_files_in_zip if not e.endswith('/')])} files
✅ <b>Registered:</b> {len(registered_files)} scripts (.py, .js)
...
⚠️ <b>Note:</b> Each script requires admin approval before running!
All supporting files (DB, configs etc.) extracted alongside scripts ✅
    ...
    try:
        ...
        if script_key in bot_scripts:
            try:
                ...
                logger.error(f"Error stopping script during delete: {e}")
        ...
        await callback.answer("✅ File deleted successfully! Script stopped if running.", show_alert=True)
        await callback_check_files(callback)
...
❤️ Total Favorites: {sum(len(favs) for favs in user_favorites.values())}
...
📅 <b>Valid Until:</b> {expiry.strftime('%Y-%m-%d')}
...
""",
            ...
            logger.error(f"Could not notify user {user_id} about premium: {e}")
...
❤️ Favorites: {user_fav_count}
💎 Account: {'Premium ✅' if premium else 'Free 🆓'}
🚀 Running: {sum(1 for k in bot_scripts if k.startswith(f"{user_id}_"))}
